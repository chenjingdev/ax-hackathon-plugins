...
description: "상품 검수/표시정보 컴플라이언스/그린워싱 판단/혼용률·원산지·시험성적서 대조/룰 갱신 소급 재스캔 요청이면 반드시 이 스킬 사용 — 상품 텍스트든 등록 화면 캡처 이미지든. 무신사 입점·운영 상품의 표시정보를 그린워싱 가이드·표시광고법·성적서에 대조해 BLOCK/WARN/PASS로 판정하고 정정안·반려 사유서·이의제기 메모·셀러 응대 초안까지 생성한다. 그린워싱 신조어(미등재 변형 표현) 의미 판단, 성적서↔상품↔클레임 정합성 4분류(정상/부분커버/불일치/외래키불명), 정책 갱신 시 전 코퍼스 소급 재스캔 포함."
---
...
**정답 참조 금지(자기오염 방지·필수)**: 검수 실행 중 `sample/EXPECTED.md`·`docs/quickstart.html`·`eval/` 등 **기대 결과가 적힌 문서를 열지 않는다**. 온보딩·퀵스타트 등으로 그런 내용이 이미 대화에 있어도 **판정 근거로 삼지 않는다** — 판정은 오직 입력(첨부 이미지/텍스트/JSON)과 `rules/ruleset.json` 대조에서만 도출하고, 리포트의 모든 claimed/observed 값은 입력에서 직접 인용한다. 기대값과 결과가 다르면 기대값이 아니라 계산 결과를 보고한다.

## 1. 입력 정규화 (§3)
입력은 두 모드. 자유 텍스트나 **상품 등록/상세 화면 캡처 이미지**로 와도 표시정보(상품명·브랜드·소재/혼용률·원산지·가격·카피·태그)를 추출해 아래 스키마로 정규화한 뒤 심사한다(예시: `sample/demo/등록화면캡처.png`). 단, **시험성적서만은 이미지가 아니라 구조화 JSON**으로 받는다(§0 OCR 비목표 — 수치 판정 신뢰성). 캡처에 성적서가 없으면 TC/MX 증빙 규칙은 "성적서 미첨부" 기준으로 평가하고, 성적서 JSON이 별도로 오면 합쳐서 심사한다. **폴백**: 검수 대상 입력이 주어지지 않으면(예: "이 상품 검수해줘"처럼 대상 없이 트리거된 경우) 플러그인 루트의 `sample/input.product.json`을 로드해 시연하고, 리포트 서두에 **번들 샘플 시연**임을 명시한다(실데이터 검수와 혼동 방지).
...
**도구가 없을 때 — 조용히 넘어가지 않는다**:
- **대화형 세션**(사용자가 후속 답을 할 수 있는 상황)이면 **리포트·출력단 생성 전에 멈추고** 알린다: "법적근거 MCP 검증층(compliance-kb)이 연결되지 않았습니다. 앱을 완전 종료(Cmd+Q) 후 재시작하거나 플러그인을 재설치하면 연결됩니다. MCP 없이 ruleset의 공개 근거 URL 인용만으로 진행하려면 '폴백으로 진행'이라고 답해 주세요." — 사용자가 명시적으로 승인한 경우에만 폴백으로 진행한다.
- **후속 질문이 불가능한 단발 실행**(codex exec 등 배치)이면 멈추지 않고 폴백으로 진행하되, 리포트 서두와 채팅에 "MCP 미사용 폴백"을 명시한다.
- 폴백 인용은 ruleset `evidence`(E1~E6) URL을 그대로 쓴다(오프라인 결정론 바닥 불변). 근거 데이터는 `kb/`이고 정책(판정)은 여전히 ruleset이 권위다.
**사용 여부를 채팅에 명시**: 채팅 응답에 "법적근거 MCP 검증: 사용(verify_citation 통과)" 또는 "미사용(도구 미노출 — 폴백 승인됨)" 한 줄을 반드시 넣는다(§5 채팅 응답 4항). 리포트 템플릿·검증 푸터는 변경하지 않는다.
