...
3. 시연 제안 한 줄: "문서에 등록 화면이 실린 **'친환경 에코레더 무스탕 자켓'(690,000원)** — 안감을 '캐시미어 30%'라고 주장하는데 시험성적서가 없습니다. 이 상품이 걸러지는지 검수해볼까요? (기대 판정: **BLOCK**)"
...
- 마지막에 시연 제안 한 줄(상품 이야기로): "번들 샘플 상품 **'친환경 에코레더 무스탕 자켓'(690,000원)** — 안감 '캐시미어 30%' 주장에 시험성적서 미첨부 — 이 상품이 걸러지는지 검수해볼까요? (기대 판정: **BLOCK**)"

### 4. 시연 수락 시 — 파일은 사용자가 직접 첨부하게
**바로 번들 샘플을 로드해 돌리지 않는다** — 그러면 사용자에게는 실제 계산과 미리 완성된 결과 호출이 구분되지 않는다. 대신:
1. macOS면 `open -R "<루트>/sample/input.product.png"`로 시연용 캡처 파일을 Finder에서 선택된 상태로 보여준다(다른 OS면 절대 경로를 채팅에 제시).
2. 채팅으로 안내: "Finder에 띄워드린 등록 화면 캡처를 이 채팅에 끌어다 놓고 **'검수해줘'**라고 해주세요. 첨부하신 이미지에서 표시정보를 추출해 규칙 대조를 수행합니다."
