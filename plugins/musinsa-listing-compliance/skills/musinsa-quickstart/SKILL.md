...
### 3. 채팅에 핵심 요약
`docs/quickstart.html`을 직접 읽어 다음을 채팅으로 요약한다:
- **용도 1~2줄**: 무신사 입점·운영 상품의 표시정보(혼용률·원산지·친환경 표현·시험성적서) 컴플라이언스를 변경 이벤트마다 검수·재판정하는 에이전트.
...
### 4. 다음 행동 제안
"번들 샘플 `sample/input.product.json`으로 바로 시연해볼까요? (기대 판정: **BLOCK**)"이라고 제안한다. 사용자가 수락하면 **listing-compliance** 스킬 절차로 넘겨 검수를 진행한다(이 스킬은 검수를 직접 수행하지 않는다).
