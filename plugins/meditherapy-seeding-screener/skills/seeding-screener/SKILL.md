...
- **정답 참조 금지(자기오염 방지)**: 스크리닝 실행 중 `sample/EXPECTED.md`·`docs/quickstart.html`·`eval/` 등 기대 결과가 적힌 문서를 열지 않는다. 온보딩 등으로 그런 내용이 이미 대화에 있어도 판단 근거로 삼지 않는다 — 점수·인용은 오직 입력 콘텐츠와 `ontology.json` 대조, `scoring/cli.py` 실행 결과에서만 나온다. 기대값과 결과가 다르면 기대값이 아니라 계산 결과를 보고한다.
