...
흐름: 입력(§3.2/§3.4) + ontology + 에이전트 부합도(JSON) →
...
사용:
  python3 -m scoring.cli score --input sample/input.kr.json \
      --ontology rules/ontology.json --agentic agentic.json \
...
def _score_candidate(creator, product_ctx, ontology, agentic_for_cid, deterministic_only):
    norm = normalize(creator, product_ctx)
    det = extract_deterministic_features(norm, ontology)
    cid = norm["creator_id"]
    ...
    res = compute(det, agentic_fits, ontology)
    return {
        "creator_id": cid,
...
def run_score(input_path, ontology_path, agentic_path, deterministic_only):
    ...
    scored = [
        _score_candidate(cr, product_ctx, ontology, agentic.get(cr.get("creator_id", "")), deterministic_only)
        for cr in data.get("candidates", [])
    ]

    result = {
        "product": product_ctx["product"].get("name", ""),
        "ruleset_version": ontology.get("ruleset_version", "v0.1"),
        "guard": ontology.get("guard"),
        "mode": mode,
        "candidates": scored,
    }
    ...
    if mode == "rescan":
        ...
        result["rescan"] = {
            ...
            "to": data.get("ruleset_version_to", ontology.get("ruleset_version", "")),
            "deltas": deltas,
        }
    return result
...
def main(argv=None):
    ...
    sc.add_argument("--ontology", default=os.path.join(_SRC, "rules", "ontology.json"))
    ...
    if args.cmd == "score":
        result = run_score(args.input, args.ontology, args.agentic, args.deterministic_only)
