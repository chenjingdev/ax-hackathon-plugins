...
ontology 동결 가중치로 차원별 값에서 산출한다. 에이전트는 차원별 0~1 부합도만 매긴다.
블렌드비·가중치·D1 direct_weight는 ontology.json 동결값(루프 패치 금지·가중치 backdoor 차단).
"""
...
def combine_dimensions(det_features, agentic_fits, ontology):
    """차원별 dimension_score = blend(det, agentic). D1은 direct_weight 감쇠(매개 재배분)."""
    blend = ontology["blend"]
...
def purchase_intent_fit(dimension_scores, ontology):
    """고정 공식 합산 → 0~100 정수. ontology 차원만 순회(에이전트 total 무시)."""
    s = 0.0
    for dim in ontology["dimensions"]:
        D = dim["id"]
...
def compute(det_features, agentic_fits, ontology):
    """normalize→features 이후 한 후보의 차원 점수 + 총점 산출."""
    dims = combine_dimensions(det_features, agentic_fits, ontology)
    return {
        "dimensions": dims,
        "purchase_intent_fit": purchase_intent_fit(dims, ontology),
    }
