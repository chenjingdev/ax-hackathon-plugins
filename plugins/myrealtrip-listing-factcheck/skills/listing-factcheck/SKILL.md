...
## 1. 입력 정규화 (§3·§5.1)
검수 대상 입력이 없으면 플러그인 루트 `sample/`의 대표 샘플(`input.rescan.json` 재스캔 킬러 데모)을 로드해 시연하고, 리포트 서두에 **"번들 샘플 시연"**임을 명시한다.
...
1. **산출 경로 한 줄** — 리스팅 주장을 동결 공식사실 스냅샷에 규칙 대조(결정론 하드게이트)해 나온 판정임을 밝힌다.
2. **판정별 "주장 vs 공식사실" 대조 1줄 + 규칙 ID·근거 URL** — 예: '매주 월요일 출발' 주장 vs 오르세 월요일 정기 휴관 → {OD-01} BLOCK. 값은 방금 생성한 JSON 로그의 `violations[].claimed/official_fact/source_url`에서 옮긴다(문서·기대값에서 옮기지 않는다).
3. 재스캔 모드면 **어떤 사실 변화(facts_delta)가 몇 건을 뒤집었는지** 1줄씩.
